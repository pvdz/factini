...
See below for image / frame related properties
...
* "gen": Means it was generated by the spriter. Will drop all frames parsed so far and start with the first frame again. (Basically allows you to overwrite the frames while keeping other meta data)
* "unused": Means the image should not be loaded as it is not actually used, only defined. Also means it will be excluded from the sprite map generator.
* "title": Comment. Ignored.
...
* "machine": Definition for woops (parts you need to build). Syntax is \`- machine: [\\dx\\d] [name]\`, like \`- machine 3x3 Asset_Machine\`. Defaults to smallest machine with enough ports.

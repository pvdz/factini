...
pub fn get_fresh_quest_states(options: &Options, state: &State, config: &Config, ticks: u64, available_parts: &Vec<PartKind>, initial_finished_quests: Vec<String>) -> Vec<QuestState> {
  log!("get_fresh_quest_states(options.trace_quest_status={}, options.dbg_no_initial_finished_quests={})", options.trace_quest_status, options.dbg_no_initial_finished_quests);
  ...
  let mut quests: Vec<QuestState> = config.stories[state.active_story_index].quest_nodes.iter().enumerate().map(|(quest_index, &quest_node_index)| {
    ...
    let forced = if options.dbg_no_initial_finished_quests { false } else { initial_finished_quests.contains(&config.nodes[quest_node_index].name) };
    if options.trace_quest_status && forced { log!("- {} ({}) is immediately finished by the map, options.dbg_no_initial_finished_quests={}", quest_node_index, config.nodes[quest_node_index].raw_name, options.dbg_no_initial_finished_quests); }
    let status =
      if forced {
        QuestStatus::Finished
      } else {
        quest_get_status_first_pass(options, state, config, quest_node_index, available_parts, quest_index)
      };
  ...
  // Remove all finished quests from the todos list of other. Anything Waiting with no todos left is now Active

  if options.trace_quest_status { log!("pruning todo lists..."); }
  for quest_index in 0..quests.len() {
    let status = quests[quest_index].status;
    if status == QuestStatus::Finished { continue; }

    if options.trace_quest_status { log!("    - Quest {} (`{}`) is {:?}, waiting for {:?}", quest_index, quests[quest_index].name, status, quests[quest_index].unlocks_todo); }

    for i in (0..quests[quest_index].unlocks_todo.len()).rev() {
      let unlock_quest_index = quests[quest_index].unlocks_todo[i];
      if options.trace_quest_status { log!("      - dep {} index {} has status {:?}", i, unlock_quest_index, quests[unlock_quest_index].status); }
      if quests[unlock_quest_index].status != QuestStatus::Waiting && quests[unlock_quest_index].status != QuestStatus::Active {
        quests[quest_index].unlocks_todo.remove(i);
      }
    }

    if options.trace_quest_status { log!("      - now waiting for {:?}", quests[quest_index].unlocks_todo); }
    if quests[quest_index].unlocks_todo.len() == 0 {
      if options.trace_quest_status { log!("    --> quest {} is now active...", quest_index); }
      // All quests this quest depends on are finished so this quest is active
      quest_update_status_sans_factory(&mut quests[quest_index], QuestStatus::Active, 0);
    }
  }
...
pub fn quest_reset_progress(options: &Options, state: &State, config: &Config, factory: &mut Factory) {
  ...
  factory.trucks = vec!();
  factory.quests = get_fresh_quest_states(options, state, config, 0, &all_available_in_this_story.iter().map(|(kind, _visible)| *kind).collect(), vec!());

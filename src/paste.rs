    ...
    for y in 0..clipboard_h {
      for x in 0..clipboard_w {
        ...
        if is_middle(cx as f64, cy as f64) {
          let coord = to_coord(cx, cy);
          log!("patching edge: {} {} -> {}", x, y, coord);
          log!("  fixing belt {:?} {:?} {:?} {:?}", factory.floor[coord].port_u, factory.floor[coord].port_r, factory.floor[coord].port_d, factory.floor[coord].port_l);
          ...
          log!("Skipping {} {} because they are not in the middle of the floor", cx, cy);
        }
      }
    }
...
pub fn paste_one_cell(options: &mut Options, state: &mut State, config: &Config, factory: &mut Factory, x: usize, y: usize, w: usize, h: usize, cx: usize, cy: usize, coord: usize) {
  log!("- paste_one_cell({} {}) coord {} {} middle? {}", x, y, cx, cy, is_middle(cx as f64, cy as f64));

  if is_middle(cx as f64, cy as f64) {
    // Are we destroying a machine? Then destroy it proper.
    if factory.floor[coord].kind == CellKind::Machine {
      log!("  - Dropping entire machine at target cell");
    ...
    if state.selected_area_copy[y][x].kind == CellKind::Belt {
      ...
      if x == w-1 || cx == FLOOR_CELLS_W-2 {
        log!("    - Right row in copy area. Clear rightside left port; {:?}", factory.floor[to_coord_right(coord)].port_l);
      ...
      if y == h-1 || cy == FLOOR_CELLS_H-2 {
        log!("    - Down row in copy area. Clear lower up port; {:?}", factory.floor[to_coord_down(coord)].port_u);
      ...
      if x == 0 || cx == 0 {
        log!("    - Left row in copy area. Clear leftside right port; {:?}", factory.floor[to_coord_left(coord)].port_r);
...
pub fn belt_connect_up_if_either_has_port_fix_partial(options: &mut Options, state: &mut State, config: &Config, factory: &mut Factory, coord: usize) {
  ...
  if let Some(ocoord) = factory.floor[coord].coord_u {
    log!("      - connect_up_if_either_has_port_fix_partial({:?}) from:{:?} to:{:?}", factory.floor[coord].kind, factory.floor[coord].port_u, factory.floor[ocoord].port_d);
    ...
    match factory.floor[ocoord].kind {
      CellKind::Belt => {
        ...
        // make them both unknown, and otherwise they will are already be set to none.
          ...
          if factory.floor[ocoord].port_d != Port::Outbound {
            log!("        - Setting up.port_d to outbound");
          ...
          if factory.floor[ocoord].port_d != Port::Inbound {
            log!("        - Setting up.port_d to inbound");
          ...
          log!("        - Setting both ports to unknown");
          ...
          log!("        - both ports are already unknown or none; {:?} {:?}", factory.floor[coord].port_u, factory.floor[ocoord].port_d);
          assert!(
      ...
      CellKind::Supply => {
        ...
        log!("        - Up is supply so this.port_u must be inbound");
        factory.floor[coord].port_u = Port::Inbound;
      }
      CellKind::Demand  => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Up is demand so this.port_u must be outbound");
        factory.floor[coord].port_u = Port::Outbound;
      }
      CellKind::Machine => {
        // This only depends on the belt since the machine port is auto
        log!("        - Up is machine so it depends on this port: {:?}", factory.floor[coord].port_u);
      ...
      CellKind::Empty => {
        log!("        - Up is empty so clear this port_u, which was {:?}", factory.floor[coord].port_u);
...
pub fn belt_connect_right_if_either_has_port_fix_partial(options: &mut Options, state: &mut State, config: &Config, factory: &mut Factory, coord: usize) {
  ...
  if let Some(ocoord) = factory.floor[coord].coord_r {
    log!("      - connect_right_if_either_has_port_fix_partial({:?}) from:{:?} to:{:?}", factory.floor[coord].kind, factory.floor[coord].port_r, factory.floor[ocoord].port_l);
    ...
    match factory.floor[ocoord].kind {
      CellKind::Belt => {
        ...
        // make them both unknown, and otherwise they will are already be set to none.
          ...
          if factory.floor[ocoord].port_l != Port::Outbound {
            log!("        - Setting right.port_l to outbound");
          ...
          if factory.floor[ocoord].port_l != Port::Inbound {
            log!("        - Setting right.port_l to inbound");
          ...
          log!("        - both ports are already unknown or none; {:?} {:?}", factory.floor[coord].port_r, factory.floor[ocoord].port_l);
          assert!(
      ...
      CellKind::Supply => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Right is supply so this.port_r must be inbound");
        factory.floor[coord].port_r = Port::Inbound;
      }
      CellKind::Demand  => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Right is demand so this.port_r must be outbound");
        factory.floor[coord].port_r = Port::Outbound;
      }
      CellKind::Machine => {
        // This only depends on the belt since the machine port is auto
        log!("        - Right is machine so it depends on this port: {:?}", factory.floor[coord].port_r);
      ...
      CellKind::Empty => {
        log!("        - Right is empty so clear this port_r, which was {:?}", factory.floor[coord].port_r);
...
pub fn belt_connect_down_if_either_has_port_fix_partial(options: &mut Options, state: &mut State, config: &Config, factory: &mut Factory, coord: usize) {
  ...
  if let Some(ocoord) = factory.floor[coord].coord_d {
    log!("      - connect_down_if_either_has_port_fix_partial({:?}) from:{:?} to:{:?}", factory.floor[coord].kind, factory.floor[coord].port_d, factory.floor[ocoord].port_u);
    ...
    match factory.floor[ocoord].kind {
      CellKind::Belt => {
        ...
        // make them both unknown, and otherwise they will are already be set to none.
          ...
          if factory.floor[ocoord].port_u != Port::Outbound {
            log!("        - Setting down.port_u to outbound");
          ...
          if factory.floor[ocoord].port_u != Port::Inbound {
            log!("        - Setting down.port_u to inbound");
          ...
          log!("        - both ports are already unknown or none; {:?} {:?}", factory.floor[coord].port_d, factory.floor[ocoord].port_u);
          assert!(
      ...
      CellKind::Supply => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Down is supply so this.port_d must be inbound");
        factory.floor[coord].port_d = Port::Inbound;
      }
      CellKind::Demand  => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Down is demand so this.port_d must be outbound");
        factory.floor[coord].port_d = Port::Outbound;
      }
      CellKind::Machine => {
        // This only depends on the belt since the machine port is auto
        log!("        - Down is machine so it depends on this port: {:?}", factory.floor[coord].port_d);
      ...
      CellKind::Empty => {
        log!("        - Down is empty so clear this port_d, which was {:?}", factory.floor[coord].port_d);
...
pub fn belt_connect_left_if_either_has_port_fix_partial(options: &mut Options, state: &mut State, config: &Config, factory: &mut Factory, coord: usize) {
  ...
  if let Some(ocoord) = factory.floor[coord].coord_l {
    log!("      - connect_left_if_either_has_port_fix_partial({:?}) from:{:?} to:{:?}", factory.floor[coord].kind, factory.floor[coord].port_l, factory.floor[ocoord].port_r);
    ...
    match factory.floor[ocoord].kind {
      CellKind::Belt => {
        ...
        // make them both unknown, and otherwise they will are already be set to none.
          ...
          if factory.floor[ocoord].port_r != Port::Outbound {
            log!("        - Setting Left.port_r to outbound");
          ...
          if factory.floor[ocoord].port_r != Port::Inbound {
            log!("        - Setting Left.port_r to inbound");
          ...
          log!("        - both ports are already unknown or none; {:?} {:?}", factory.floor[coord].port_l, factory.floor[ocoord].port_r);
          assert!(
      ...
      CellKind::Supply => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Left is supply so this.port_l must be inbound");
        factory.floor[coord].port_l = Port::Inbound;
      }
      CellKind::Demand  => {
        // Can only be one way and not sure why we wouldn't connect it immediately
        log!("        - Left is demand so this.port_l must be outbound");
        factory.floor[coord].port_l = Port::Outbound;
      }
      CellKind::Machine => {
        // This only depends on the belt since the machine port is auto
        log!("        - Left is machine so it depends on this port: {:?}", factory.floor[coord].port_l);
      ...
      CellKind::Empty => {
        log!("        - Left is empty so clear this port_l, which was {:?}", factory.floor[coord].port_l);
...
pub fn belt_fix_semi_connected_ports_partial(options: &Options, state: &State, factory: &mut Factory, coord: usize) {
  ...
  log!("belt_fix_semi_connected_ports_partial({})", coord);
